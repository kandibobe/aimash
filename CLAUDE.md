...
9. **Замок аккаунта (раздельно ЧТЕНИЕ и МУТАЦИИ).** МУТАЦИИ по умолчанию — ТОЛЬКО на `Aimash (Draft)` = **`7753643025`** (775-364-3025) — чокпойнт `ads.client.ensure_allowed(customer_id)`, пустой мутационный allow-list = **fail-closed**. **Точный контракт потолка (уточнено 2026-07):** КОД-минимум `ALLOWED_CEILING = {Draft}` нельзя убрать/понизить через env; ЭФФЕКТИВНЫЙ потолок `allowed_ceiling()` = этот минимум ∪ аккаунты, ВИДИМЫЕ боту (env read-list + дочерние, обнаруженные обходом MCC). Включить мутации на не-Draft аккаунте (управляемый список, решение владельца) можно КОНФИГОМ, но только при ДВУХ условиях: (1) аккаунт видим боту и (2) явно добавлен в `GOOGLE_ADS_ALLOWED_CUSTOMER_IDS` — опечатка в чужой боевой id отсекается (его нет среди видимых). То есть «env не расширит потолок» верно для константы `ALLOWED_CEILING`, но эффективный набор управляется конфигом в этих жёстких рамках; по умолчанию `allowed_customer_ids` пуст ⇒ мутаций нет вовсе (строже). Инварианты: `test_mutation_lock_unchanged_by_read_allowlist`, `test_mutation_enabled_for_visible_account_in_allowlist`. ЧТЕНИЕ per-account — ОТДЕЛЬНЫЙ чокпойнт `ensure_read_allowed(customer_id)`: множество = мутационный список ∪ `GOOGLE_ADS_READ_CUSTOMER_IDS` (env) ∪ **дочерние, обнаруженные обходом MCC на старте** (`ads.client.discover_read_children` под `ensure_manager_allowed`; §8-полный-мульти-аккаунт). Все три пусты = **fail-closed**. Это позволяет §8 (сводка по дочерним MCC) читать дочерние, **НЕ открывая мутаций на них** (мутация на дочернем всё равно упрётся в `ensure_allowed`; инварианты `test_mutation_lock_unchanged_by_read_allowlist`, `test_discovered_child_readable_but_not_mutable`). **Исполнение привязано к черновику (аудит 2026-07):** `execute_confirmed` берёт аккаунт из `proposal.customer_id` и ЗАНОВО проходит `ensure_allowed` на исполнении (инварианты `tests/test_execute_account_binding.py`). Поверх чтения — **пер-пользовательский грант** (`core.access`/`account_access`, режимы `ACCOUNT_ACCESS_MODE=auto|enforced|legacy`; `/grant`/`/revoke` — админы `ADMIN_CHAT_IDS`); грант чтения НЕ открывает мутации (`test_grant_does_not_open_mutations`). **Текущее состояние §8 — РЕАЛИЗОВАНО:** авто-обход дочерних на старте + команда `/mcc` по ВСЕМ настроенным MCC + подытоги по валютам (без FX, golden rule 4) + нормализация таймзон per-child + per-account OAuth-токены (`load_oauth_cache`). МУТАЦИИ по-прежнему только на Draft.
...
- Не работать против боевых аккаунтов. **По умолчанию мутируется ТОЛЬКО `7753643025` (Draft)** — при пустом `allowed_customer_ids` мутаций нет вовсе (fail-closed). Мутации на не-Draft — только УПРАВЛЯЕМЫМ конфигом (`GOOGLE_ADS_ALLOWED_CUSTOMER_IDS`), и только на аккаунт, ВИДИМЫЙ боту (⊆ `allowed_ceiling()`); это осознанное решение владельца, а не «любой id, кроме Draft, запрещён навсегда» (точный контракт — золотое правило 9 и шапка `ads/client.py`).
