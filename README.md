...
| 3 · READ-инструменты | ✅ 23 READ-инструмента из 38 по реестру §6.1. Round-trip через Hermes доказан живьём на 15 из них; 8 добавленных 30.07 покрыты только офлайн-смоуком (`tests/test_mcp_read_smoke.py`, SDK подменён) |
...
Точный статус — по `git log` и [`docs/ACCEPTANCE.md`](docs/ACCEPTANCE.md), **не** по план-файлам: они отстают.
...
mcp_server/   тул-слой для Hermes: 23 READ ✅, MEMORY/PLAN/WRITE — Волна 1        ← пишется заново
