...
| **Переходное сейчас** | Hermes подключён к MCP через контейнер `aimash-bot`; trusted transport принят | 25 READ + flag-gated 40 PLAN + 1 WRITE; исполнение только по HMAC-доверенному reply-якорю |
...
| 5 · Мутации + реплай-гейт | ✅ 40 PLAN + 1 WRITE регистрируются только при `HERMES_WRITE_ENABLED=true`; каждый вызов проходит HMAC trusted transport и reply-CAS, кнопочный путь остаётся переходным fallback |
...
mcp_server/   тул-слой Hermes: 25 READ + flag-gated 40 PLAN + 1 WRITE через trusted transport
