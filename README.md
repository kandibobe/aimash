...
## Статус
**Идёт пивот на Hermes: функциональный объём сохраняется, меняется архитектура** — кнопочный интерфейс уступает свободному тексту с подтверждением реплаем (`SPEC.md` §2). Волна 0 (прототипы) ✅, MCP READ ✅; следующее — Волна 1 шаги 1–10 (§12). Точный статус — по `git log` и `docs/ACCEPTANCE.md`, не по план-файлам.
