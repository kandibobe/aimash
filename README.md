...
## Статус
**Идёт пивот на Hermes: функциональный объём сохраняется, меняется архитектура** — основной ввод свободным текстом, подтверждение карточки кнопкой или привязанным reply (`SPEC.md` §2).
...
| **Переходное сейчас** | Hermes подключён к MCP через контейнер `aimash-bot`; trusted transport принят | 38 READ + 1 META + flag-gated 61 PLAN/state + 1 WRITE; кнопка и reply сходятся в HMAC-доверенном якоре |
...
| Шаг | Состояние |
|---|---|
| 1 · Hermes на VPS + bot-free bootstrap | ✅ пин v0.19.0, тулсет-поверхность заперта, `bot/`-зависимости срезаны, планировщик отдельным процессом; Telegram gateway активен |
| 2 · Гарды И1–И5 | ✅ И1–И8 покрыты construction/runtime-тестами; skipped-гардов в `tests/test_hermes_isolation.py` нет |
| 3 · READ-инструменты | ✅ 38 READ-инструментов. Round-trip через Hermes доказан живьём на базовой поверхности; workflow-ридеры покрыты офлайн-смоуком (`tests/test_mcp_read_smoke.py`, SDK подменён) |
| 4 · Память | 🟡 `recall_client` есть; `remember_fact`/`agent_facts` — нет |
| 5 · Мутации + confirm-гейт | ✅ 61 PLAN/state + 1 WRITE регистрируются только при `HERMES_WRITE_ENABLED=true`; операционный state требует RBAC, кнопка карточки и reply проходят HMAC trusted transport и единый reply-CAS |
...
mcp_server/   тул-слой Hermes: 38 READ + 1 META + flag-gated 61 PLAN/state + 1 WRITE через trusted transport
