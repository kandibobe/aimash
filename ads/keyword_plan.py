...
# A1: нейтральные дефолты — БЕЗ гео/языкового биаса. Раньше было "ru"/(2804 Украина), и это
# молча искажало объёмы/CPC для любого не-украинского клиента (реальный клиент льёт на Уганду).
# Пусто ⇒ generate_keyword_ideas не задаёт languageConstant/geoTargetConstants → глобальный подбор.
# «Домашнюю» страну (если агентство всегда на одну страну) подставляет ОРКЕСТРАТОР из
# settings.geo_default_country (bot.main._kw_run), а не этот низкоуровневый API-обёртчик.
DEFAULT_LANGUAGE = ""
