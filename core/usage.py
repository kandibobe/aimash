...
(`llm.router` брал только message). Теперь агрегируем по ролям — чтобы в боте (/balance)
...
(`llm.openrouter_account`, эндпоинты /credits и /key): он переживает рестарты. Этот модуль —
...
def record(role: str, usage: Any) -> None:
    ...
    проблему разбора usage глотаем молча (вызывается из llm.router в обёртке try)."""
    try:
        v = extract(usage)
