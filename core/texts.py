"""Тексты и шаблоны сообщений (RU/EN). Вынесены отдельно — упрощает правки и будущую
...
Живёт в `core/` вместе с `core.i18n` — пара неразделима (i18n мостит к texts.<KEY>), а зовут её
и фоновые контуры без aiogram. Подробнее о причине переезда — докстринг `core/i18n.py`.
"""
...
# Рендер черновика мутации уехал в confirm/render.py (Ш1) — имена остались доступны как
# texts.* ради вызывающих; сам текст обязан быть одинаков в Telegram и headless-контуре.
from confirm import render
from confirm.render import KW_INLINE_MAX, _thou
...
def _lang(lang: str | None) -> str:
    ...
    ставит LangMiddleware). i18n импортируем ЛЕНИВО внутри функции — core.i18n импортирует
    core.texts на уровне модуля, поэтому верхнеуровневый импорт здесь дал бы циклический import."""
    if lang is None:
...
def status_human(status: str, lang: str | None = None) -> str:
    """Статус словами. Реализация — `confirm.render.status_human` (Ш1: рендер «было → станет»
    вынесен из Telegram-слоя); здесь только разрешение языка из contextvar запроса."""
...
# Денежный путь: таймаут/INTERNAL/DEADLINE во время SDK — мутация МОГЛА примениться на сервере
# (asyncio.timeout не отменяет воркер to_thread). НЕ «не удалось» (это спровоцировало бы повтор →
# дубль кампании/бюджета): честно говорим «исход неизвестен, сверьте перед повтором».
NEEDS_REVIEW = (
    "⚠️ <b>Исход неизвестен.</b> Google Ads не ответил вовремя — изменение "
    "<b>могло примениться</b>. Сверьте аккаунт в Google Ads <b>перед повтором</b> "
    "(иначе можно задвоить): {err}"
)
REJECTED = "❌ Отменено"
...
# Реализация — `confirm/render.py` (headless, без aiogram). Здесь остались обёртки, которые
# разрешают язык текущего запроса из contextvar, и telegram-специфичные подписи кнопок.
def match_type_human(mt: str, lang: str | None = None) -> str:
    """Тип соответствия словами. Реализация — `confirm.render.match_type_human`."""
    return render.match_type_human(mt, _lang(lang))
