"""Волна 5: тир риска меняет форму и может ужесточить гейт, но никогда его не ослабляет.

Исходная формулировка требования была «L1 — применять без человека». Реализовано иначе: тир решает
ФОРМУ вопроса (полнота карточки, вложение с графиком, срок жизни согласия) и при отдельно
включённом four-eyes требует независимый голос для выбранных тиров, а не решает, нужен ли вопрос.
Разница не косметическая — классификатор может ошибиться, и при
...
верно, тир физически не может попасть ни в `ensure_allowed`, ни в провенанс, ни в одну из девяти
базовых проверок §2.2. Дополнительные TTL/four-eyes-конъюнкты находятся в `ConfirmStore.claim` и
могут только отказать в исполнении — не открыть его.
