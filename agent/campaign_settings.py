...
def assemble_settings(
    ...
    *,
    ...
    # Валюта: АККАУНТНАЯ авторитетна (ревью 2026-07-07: Google бронирует деньги в валюте аккаунта —
    # показывать пользовательскую «йен» на USD-аккаунте значило бы врать в сводке); явная из
    # описания — только когда валюта аккаунта неизвестна (тест-аккаунты без меты). Денежные ДЕФОЛТЫ
    # валюто-зависимые (0.5 JPY за клик — абсурд); явные значения пользователя не пересчитываются
    # (golden rule #4: деньги считает КОД).
    currency = (
        (account_currency or "").strip().upper()
        or (extracted.currency or "").strip().upper()
        or None
    )
