...
> ⚠️ **Но «бинарь известен» ≠ «ключи сверены»:** аттестация ключей ниже и в `config.yaml` делалась против доков
> **v0.17**, и после V1 её никто не пересверял. Платформа 0.x с миграциями конфига между версиями + К10
> («молча игнорирует») = ровно тот случай, где ключ мог перестать существовать без единого сообщения; локальный
> прогон `lint_config.py` сейчас отдаёт **10 верхнеуровневых ключей `[НЕ АТТЕСТОВАН]`**. Поэтому: **после любой
> правки конфига** проверяй, что значение реально
...
инструментов [Likely]. Чтобы позитивно убедиться, что 23 READ-инструмента живые — в чате `/reload-mcp` и спросить агента
...
| V | Действие | Проверка |
|---|---|---|
| **V18** | Удалить `~/.hermes/plugins/aimash_probe`, **убрать `aimash_probe` из `plugins.enabled`**, удалить запись `mcp_servers.aimash-probe`, все `AIMASH_PROBE_*` из окружения, **вернуть `vision` в `disabled_toolsets`** (если снимался под R6); restart | `hermes plugins` → `aimash_probe` отсутствует; `hermes mcp list` → только `aimash`; `hermes config show` → 23 READ-инструмента, `vision` погашен; `hermes gateway status` → active |
