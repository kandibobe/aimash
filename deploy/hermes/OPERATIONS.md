...
> валидацию). Все факты ниже сверены с офиц. доками, но доки — ветка `main`, а **какая версия стоит на VPS —
> НЕ ИЗМЕРЕНО**: пин проекта — `v0.19.0` (тег `v2026.7.20`, `deploy/hermes/PIN.json`), эта шапка исторически
> говорила `v0.17`, и разрешает противоречие замер **V1** (§12). До него любое «сверено» означает «сверено с
> докой пина», а не «сверено с бинарём». Команда/
...
|---|---|---|---|
| **V1** | Версия бинаря | `hermes version` | **Гипотезы две и они противоречат друг другу — это и есть предмет замера.** Пин проекта — `v0.19.0`, тег `v2026.7.20` (`SPEC.md:777`, `CLAUDE.md`); шапка этого ранбука (:9) и шапка `config.yaml` говорят `v0.17`. Что бы ни показал бинарь, **вторую цифру править в документах**, а не оставлять обе. Любой номер, кроме подтверждённого, ⇒ пересверить ВСЕ ключи конфига по К10. На V1 висят три решения об объёме: `SPEC.md:225`, `:661`, `:785` |
