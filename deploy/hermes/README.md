...
MCP-сервер `aimash` (пакет `mcp_server/`, 23 READ-инструмента) читает Google Ads. Денежное ядро не
...
hermes mcp test aimash             # должен отдать 23 READ-инструмента
...
MCP READ-слой (`mcp_server/`) отдаёт чистые редактированные конверты на всех 23 READ-инструментах, но
