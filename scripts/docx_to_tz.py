"""Сборка `ТЗ.md` из трёх нормативных оригиналов заказчика (.docx).

Три оригинала совместно образуют договорный продуктовый канон. Скрипт делает их читаемое зеркало
воспроизводимым: заказчик прислал новую редакцию .docx — регенерируем.
...
>
> **ТЕКСТОВОЕ ЗЕРКАЛО ДОГОВОРНОГО КАНОНА.** Нормативны три исходных `.docx` ниже; при расхождении
> этого Markdown, `SPEC.md`, unified-артефакта или кода с оригиналом исправляется производный слой.
...
Производная матрица реализации находится в `SPEC.md` §19 и не переопределяет исходные требования.
