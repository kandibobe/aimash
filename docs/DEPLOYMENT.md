...
- **Алерт о правках МИМО бота (Р6, 2026-07-30).** `EXTERNAL_CHANGES_INTERVAL_HOURS>0` ⇒ scheduler
  опрашивает `change_event` по всем плановым аккаунтам и шлёт операторам дайджест «кто · когда ·
  каким каналом · что правил». Google о таких правках не уведомляет, а отмена возвращает настройку,
  но не потраченные деньги (`deploy/hermes/RISK_REGISTER.md`, Р6). Первый прогон по (чат, аккаунт)
  только ставит базлайн; курсор «уже показано» двигается ТОЛЬКО после доставки. Правки по каналу
  `GOOGLE_ADS_API` не показываем — свои мутации репортятся из audit-row, число отсеянных в логе.
