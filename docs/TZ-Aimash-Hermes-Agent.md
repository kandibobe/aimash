...
| «MCP-серверы — на будущее (best practice)» | MCP — **уже реальный и единственный канал** Hermes→Google Ads: READ-only, 12 инструментов, через `docker exec -i aimash-bot python -m mcp_server` ([config.yaml:213-236](../deploy/hermes/config.yaml)). WRITE физически отсутствует by construction. | Строим WRITE-MCP поверх готового `execute_confirmed`, а не «на будущее». |
...
│  • READ-MCP: 12 инструментов, read-only BY CONSTRUCTION       │
...
- **Переиспользуем:** READ-MCP (12 инструментов, envelope+error-codes, redaction); ~41 Google Ads skill (`ads/service.py` `SUPPORTED_OPERATIONS`), резолверы, post-apply verify.
