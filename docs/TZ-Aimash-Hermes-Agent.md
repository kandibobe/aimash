...
> При этом текущий прод остаётся переходным: 24 READ-инструмента опубликованы через MCP,
...
| «MCP-серверы — на будущее (best practice)» | MCP — **уже реальный и единственный канал** Hermes→Google Ads: 24 READ-инструмента через `docker exec -i aimash-bot python -m mcp_server` ([config.yaml:213-236](../deploy/hermes/config.yaml)). На live-поверхности WRITE отсутствует by construction; два `propose_*` и reply-CAS существуют ready-dark. | Достраиваем и публикуем полный WRITE-MCP поверх готового `execute_confirmed`, а не откладываем MCP «на будущее». |
...
Пользователь (Telegram)
        │  текст + reply-подтверждение (кнопки — только legacy)
        ▼
┌─────────────────────────────────────────────────────────────┐
...
│  • live: 24 READ-инструмента, read-only BY CONSTRUCTION      │
...
└───────────────┬──────────────────────────────────────────────┘
                │  gRPC (google-ads SDK → API v25), сеть только отсюда
                ▼
...
- **Достраиваем:** доверенный проброс `actor_user_id`/`actor_chat_id`/`reply_to_message_id` из Telegram gateway мимо LLM, запись id отправленной карточки в живом контуре, проброс провенанса `origin_human_turn` для агентного actor и live-регистрацию. Карточка approval: `🎯 действие / 📊 обоснование / ⚠️ риски`; целевое подтверждение — reply-текстом. Кнопки остаются только в legacy-интерфейсе, а безопасностью не являются.
