...
> `Hermes → MCP → ads/confirm`. Hermes WRITE включён только вместе с И1–И8, доверенными
> Telegram-метаданными и fail-closed reply-якорем. Inline-кнопка карточки — UX над тем же якорем:
> callback не вызывает SDK напрямую, а создаёт trusted reply-event; без HMAC/CAS WRITE не проходит.
...
2. **Файл-флаг** `KILL_SWITCH_FILE` (дефолт `KILL_SWITCH`, относительный к cwd процесса; в Compose
   общий `/run/aimash-safety/KILL_SWITCH`): `touch /opt/aimash/runtime/safety/KILL_SWITCH` на хосте
   останавливает bot/Hermes MCP и scheduler немедленно, `rm` — включает обратно. `OSError` при
   проверке ФС = ВКЛЮЧЁН. Пустое значение env отключает файловый канал осознанно. Флаг переживает
   recreate, потому что его источник — host bind; устойчивый дубль остаётся в env-канале.
