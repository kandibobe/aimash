...
> **[legacy-референс]** Восемь FSM-этапов, callback-имена и кнопки описывают текущую aiogram-реализацию, но не
> целевую архитектуру. В Hermes переносятся поля кампании, персистентный черновик, валидаторы, батч,
> `PAUSED`-инвариант и приёмка; порядок уточнений выбирает агент (`SPEC.md` §3.5), а не FSM.
