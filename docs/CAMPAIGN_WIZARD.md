...
- **Custom parameters вне UI** (§19.6): в визарде собирается `tracking | suffix`; третье поле
  `custom` схема/мутация поддерживают (`_validate_url_options`: латиница/цифры/`_`, ≤8 штук,
  значение ≤250), но основной ввод — урезанный.
- **Расписание / даты запуска не выводятся в сводку Этапа 1** (§19.3): применяются дефолты
  (24/7, старт сегодня). Значения всё же попадают в params, если извлечены из описания.
