...
- **Готовый Google Ads MCP:** официальный `googleads/google-ads-mcp` — **READ-only** (`search` GAQL, `get_resource_metadata`, `list_accessible_customers`), Python, Apache-2.0. Мутаций нет. Ваш собственный READ-MCP (`mcp_server/`) уже реализует 23 READ-инструмента; для WRITE строим свои (нет зрелого готового write-MCP на актуальной версии). [Certain]
