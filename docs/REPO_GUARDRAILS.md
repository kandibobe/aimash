...
| Target branches | `master` | Единственная линия. `main` была orphan-веткой (пустой merge-base), удалена 29.07 под тегами `archive/main-orphan-2026-07-29` (вся ветка) и `archive/main-p0p2-on-master` (её же p0-p2, отребейзенные на master); триггер `ci.yml` больше её не слушает |
