...
> **[legacy-референс] Это доказательная карта реализованного aiogram-контура, а не чек-лист
> готовности Hermes.** Здесь остаются ценными привязки функций к коду и тестам. Целевая
> приёмка свободного текста, MCP, реплай-гейта, скилов и памяти живёт в [`SPEC.md` §13](../SPEC.md)
> и [`HERMES_SPEC.md` §12](../deploy/hermes/HERMES_SPEC.md). Статус перехода — в корневом [`README.md`](../README.md).

Поэтому `Статус: ✅` ниже означает «функция доказана в старом контуре», а не «она уже вызывается из Hermes».
