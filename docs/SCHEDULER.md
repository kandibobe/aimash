...
- **`cleanup_stale_proposals`** — `pending`-черновики мутаций старше `PROPOSAL_TTL_HOURS`
  ...
  **Уборщик, а не гард (Волна 1.2).** Сам срок жизни энфорсит `ConfirmStore.confirm`/`claim` —
  условие возраста стоит в их `WHERE` (CAS), поэтому не отработавшая джоба больше не делает
  вчерашний черновик исполнимым. За джобой остаётся то, чего CAS не умеет: перевести мёртвую строку
  в терминальный `rejected` и освободить временные медиа §19/§11. `reject` условие возраста НЕ имеет
  намеренно — иначе уборщик не смог бы трогать ровно те строки, ради которых он есть.
