...
> Hermes всегда получает 25 READ-инструментов + 1 META. При `HERMES_WRITE_ENABLED=true` добавляются
> 46 PLAN/state + 1 WRITE; каждый state/WRITE-вызов проходит HMAC trusted Telegram transport, а исполнение —
