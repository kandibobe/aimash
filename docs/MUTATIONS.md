...
> Hermes получает только READ-поверхность. В `mcp_server/tools_write.py` уже есть два
> ready-dark `propose_*`, а CAS-якорь reply реализован в `confirm/store.py`, но до регистрации
> полного PLAN/WRITE-набора и доверенного транспорта Hermes боевые мутации через него закрыты.
