# Aimash — производный implementation profile

> **Статус:** ненормативная инженерная интерпретация трёх исходных DOCX заказчика
> **Дата:** 31.07.2026
> **Контракт:** нормативный продуктовый канон образуют `Aimash_Technical_Specification.docx`,
> `Aimash_Flow_Google_Search_4.docx` и `Информация о клиентах_1.docx`. Этот Markdown помогает
> разработке и CI, но не может отменять или упрощать требования исходных документов.
...
> **Hermes автономно читает, анализирует, исследует, выбирает tools и готовит единый пакет.
> Перед любой мутацией требуется одна общая карточка и одно явное подтверждение человека.**
...
### 3.2. Кнопки и подтверждение мутаций

Для любой изменяющей команды нормальный путь такой:
...
Для чтения, аудита, отчётов, исследований, чтения профиля клиента, файлов, решений, инцидентов,
расписаний и создания скилов в staging карточка подтверждения не показывается.

Требования к карточке мутации:

- Команда на изменение не является подтверждением. После неё агент сам готовит черновик.
...
- Любая мутация Google Ads или памяти клиента создаёт pending proposal и требует одну карточку с кнопками.
- Hermes автономно выбирает typed tools, читает контекст и собирает связанные действия в один batch, но не
  исполняет отдельные части до подтверждения.
- Неизвестная будущая операция также требует подтверждения (fail-closed).
...
13. показать единую финальную сводку и после одного подтверждения создать всё в `PAUSED`;
...
### 17.2. Одно подтверждение для каждой мутации
...
8. Rename, создание `PAUSED`-кампании, изменение профиля и любая другая мутация остаются pending до одного confirm.
9. Автономного mutation allowlist нет; неизвестная операция также требует один confirm.
...
| ТЗ-1 §5–6 | 3, 6, 15, 17 | Show-and-confirm и inline UX обязаны сохранять требования оригинала; выявленные упрощения считаются gaps |
...
## 20. Иерархия источников

Переход выполнен 31.07.2026:

1. `Aimash_Technical_Specification.docx` — основной договорный документ;
2. `Aimash_Flow_Google_Search_4.docx` и `Информация о клиентах_1.docx` — нормативные дополнения;
3. SHA-256 трёх оригиналов фиксируется в `ТЗ.md`, который служит только их текстовым зеркалом;
4. этот файл и любые unified DOCX — производные материалы и не разрешают расхождения с оригиналами;
5. при конфликте кода, skill, SOUL, runbook или производной спеки с оригиналами исправляется производный слой;
6. изменение договорного канона возможно только новой редакцией одного из трёх исходных DOCX заказчика.
