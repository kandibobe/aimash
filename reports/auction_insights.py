...
ЗАЧЕМ ФАЙЛ, а не API: имён конкурентов Google через API не отдаёт вовсе — ресурс `auction_insight`
в GAQL отсутствует, программа доступа закрыта. Выгрузка из веб-интерфейса — ЕДИНСТВЕННЫЙ легальный
путь узнать, кто именно стоит рядом в аукционе. Суррогат по нашим данным (доля показов, потери по
рангу/бюджету) уже есть — чек `competitive_pressure` (Ф5а); он отвечает «давят или нет», а этот
файл — «кто именно».
