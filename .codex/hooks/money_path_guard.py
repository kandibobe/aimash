#!/usr/bin/env python3
"""Codex lifecycle hook для напоминаний на «денежных» путях.

Читает JSON события из stdin, сопоставляет редактируемый файл с глоб-реестром
`.codex/hooks/money_paths.json` и:
- PreToolUse, критичный путь (ads/mutations.py, confirm/**, ...) → additionalContext с
  инвариантами золотых правил;
...
def _find_root(start: str) -> str:
    """Каталог проекта — ближайший вверх, содержащий .codex (иначе — start)."""
...
def main() -> None:
    ...
    cfg_path = os.path.join(root, ".codex", "hooks", "money_paths.json")
    ...
    # PreToolUse
    if is_critical:
        _emit(event="PreToolUse", context=cfg.get("critical_reminder"))
    elif is_broad:
